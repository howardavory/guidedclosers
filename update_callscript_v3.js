...
const startStr = "{formData.propertyType === 'Multi-Family' && (";
const endStr = "{formData.propertyType === 'Condo/Townhome' && (";
...
if (startIndex !== -1 && endIndex !== -1) {
              ...
              <div className="flex flex-col mb-2 animate-slideIn" style={{ marginTop: '1.5rem', animation: 'slideInUp 0.3s ease' }}>
                ...
                <div className="relative bg-black/60 backdrop-blur-md border-4 border-[#00E5FF] p-6 shadow-[6px_6px_0px_#000] text-xl font-bold text-white leading-relaxed rounded-2xl rounded-tl-none mb-6">
                  {formData.mfUnitCount === '' && (
                    <span>"Got it, a multi-unit. Just so I have exactly what I need to run the numbers on this, let's break it down unit by unit real quick. How many units are we talking total?"</span>
                  )}
                  {formData.mfUnitCount !== '' && formData.mfUnitsData.some(u => !u.occupancy || (u.occupancy === 'Tenant Occupied' && (!u.leaseType || !u.paymentStatus || !u.rentAmount))) && (
                    <span>"Got it, a multi-unit. Just so I have exactly what I need to run the numbers on this, let's break it down unit by unit real quick. For the first unit, what's the bed and bath count? ... And what kind of shape is that unit in, has it been updated recently? ... Makes sense. And is that specific unit vacant, or do you have a tenant in there right now?"</span>
                  )}
                  {formData.mfUnitCount !== '' && formData.mfUnitsData.every(u => u.occupancy && (u.occupancy !== 'Tenant Occupied' || (u.leaseType && u.paymentStatus && u.rentAmount))) && (
                    <span>"Perfect, that paints a great picture. Last thing on the rental side—are the meters split so the tenants pay their own utilities, or are you covering things like water, trash, or electric for the building?"</span>
                  )}
                    ...
                    {formData.mfUnitsData.length > 0 && formData.mfUnitsData.every(u => u.occupancy && (u.occupancy !== 'Tenant Occupied' || (u.leaseType && u.paymentStatus && u.rentAmount))) && (
                      <div className="mt-4 border-t border-gray-700 pt-4">
                        ...
                        <div className="flex flex-col gap-4 mb-4">
                          <button className={\`px-4 py-3 border-2 font-bold tracking-widest text-sm shadow-[2px_2px_0px_#fff] skew-x-[-2deg] hover:-translate-y-0.5 hover:shadow-[4px_4px_0px_#fff] transition-all duration-200 ease-out transform-gpu \${formData.mfUtilityMetering === 'Separately Metered (Tenant Pays All)' ? 'bg-[#00E5FF] text-black border-[#00E5FF] shadow-[2px_2px_0px_#00E5FF]' : 'bg-black border-white text-white'}\`} onClick={() => setFormData({...formData, mfUtilityMetering: 'Separately Metered (Tenant Pays All)', mfOwnerUtilities: [], mfUtilityCost: ''})}>Separately Metered (Tenant Pays All)</button>
  ...
  console.log('CallScript V3 Logic Injected via exact slice successfully!');
} else {
  console.log('Error: Could not find start or end index in CallScript.jsx');
}
