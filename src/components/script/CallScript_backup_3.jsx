...
export default function CallScript({ activeLead, onReturn, onFormUpdate }) {
  ...
  const computedTotalDebt = formData.freeAndClear ? 0 : (
    parseInt(formData.mortgageBalance || 0, 10) + 
    parseInt(formData.secondPosition || 0, 10) + 
    parseInt(formData.thirdPosition || 0, 10) + 
    parseInt(formData.arrearsAmount || formData.arrears || 0, 10)
  );
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className={clsx("flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all", isCompleted ? "bg-accent-success/20 text-green-600 border border-accent-success/50" : "bg-[var(--card-bg)] text-gray-800 border border-[var(--card-border)] hover:text-gray-800")}
            >
  ...
  return (
    <>
      <div className="flex flex-col h-[calc(100vh-120px)] overflow-hidden relative z-10">
        ...
        >
          <div className="flex justify-between items-center bg-[var(--card-bg)]/80 p-4 border-b-4 border-[var(--card-border)] shrink-0">
            <h2 className="font-semibold tracking-wide text-3xl text-[var(--text-base)] tracking-widest uppercase italic" style={{ textShadow: '2px 2px 0px #000' }}>
          ...
          <div className="flex-1 overflow-y-auto hide-scrollbar p-6 bg-gray-100 flex flex-col">
        ...
        {/* FLOATING ACTION TABS (Only show when drawer is closed) */}
        {!activeDrawer && (
          <div className="fixed right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-[9998]">
            <button 
              onClick={() => setActiveDrawer('calculators')}
              className="translate-x-2 hover:translate-x-0 transition-transition-all duration-300 duration-300 border border-[var(--card-border)] p-3 pl-4 rounded-l-2xl group flex flex-col items-center gap-2 bg-[var(--card-bg)]/80 shadow-md"
            >
              <Calculator size={32} color="#00E5FF" className="group-hover:text-gray-800 group-hover:animate-pulse" />
              <span className="font-semibold tracking-wide uppercase tracking-widest text-[var(--text-base)] group-hover:text-gray-800" style={{ writingMode: 'vertical-rl', textOrientation: 'mixed' }}>Calculators</span>
            </button>
            ...
            <button 
              ...
              className="translate-x-2 hover:translate-x-0 transition-transition-all duration-300 duration-300 border border-[var(--card-border)] p-3 pl-4 rounded-l-2xl group flex flex-col items-center gap-2 bg-[var(--card-bg)]/80 shadow-md"
            >
              <FileText size={32} color="#FFE600" className="group-hover:text-gray-800 group-hover:animate-pulse" />
              <span className="font-semibold tracking-wide uppercase tracking-widest text-[var(--text-base)] group-hover:text-gray-800" style={{ writingMode: 'vertical-rl', textOrientation: 'mixed' }}>Tear-Sheet</span>
            </button>
            ...
            <button 
              ...
              className="translate-x-2 hover:translate-x-0 transition-transition-all duration-300 duration-300 border border-[var(--card-border)] p-3 pl-4 rounded-l-2xl group flex flex-col items-center gap-2 bg-[var(--card-bg)]/80 shadow-md"
            >
              <BarChart2 size={32} color="#00FF66" className="group-hover:text-gray-800 group-hover:animate-pulse" />
              <span className="font-semibold tracking-wide uppercase tracking-widest text-[var(--text-base)] group-hover:text-gray-800" style={{ writingMode: 'vertical-rl', textOrientation: 'mixed' }}>Analytics</span>
            </button>
          </div>
        )}
        ...
        {hudElement && createPortal(
          <div className="flex flex-nowrap gap-4 justify-end items-start w-full h-full">
            <div className="w-[250px] shrink-0 relative h-full">
              <div className="bg-[var(--card-bg)] border border-[var(--card-border)] shadow-md transition-all duration-300 font-['Josefin_Sans'] h-full">
                <div 
                  className="bg-[var(--card-bg)]/80 flex items-center justify-between cursor-pointer p-4 relative overflow-hidden transition-transition-all duration-300 active:scale-[0.98] h-full"
                ...
                >
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <h3 className="font-semibold tracking-wide text-xl text-[var(--text-base)] tracking-widest uppercase italic" style={{ textShadow: '2px 2px 0px #000' }}>DISQUALIFY</h3>
                  </div>
                </div>
              </div>
              {showDisqualifyMenu && (
                <div className="absolute top-[calc(100%+15px)] right-0 bg-[var(--card-bg)] border border-[var(--card-border)] p-4 flex flex-col gap-3 shadow-md w-[280px] z-[1001] animate-slideIn">
                  <button className="w-full bg-[var(--card-bg)]/60 text-[var(--text-base)] border-2 border-[var(--card-border)] p-3 font-black uppercase italic tracking-widest hover:-translate-y-0.5 hover:shadow-md transition-all" onClick={() => onReturn && onReturn({ type: 'disqualified', reason: 'Not Selling', details: formData })}>DISQUALIFY: Not Selling</button>
                  <button className="w-full bg-[var(--card-bg)]/60 text-[var(--text-base)] border-2 border-[var(--card-border)] p-3 font-black uppercase italic tracking-widest hover:-translate-y-0.5 hover:shadow-md transition-all" onClick={() => onReturn && onReturn({ type: 'disqualified', reason: 'DNC', details: formData })}>DISQUALIFY: DNC</button>
                  <button className="w-full bg-[var(--card-bg)]/60 text-[var(--text-base)] border-2 border-[var(--card-border)] p-3 font-black uppercase italic tracking-widest hover:-translate-y-0.5 hover:shadow-md transition-all" onClick={() => onReturn && onReturn({ type: 'disqualified', reason: 'Hostile', details: formData })}>DISQUALIFY: Hostile</button>
                  <hr className="border-t-4 border-[var(--card-border)] my-1" />
                  <button className="w-full bg-[var(--card-bg)]/60 text-gray-800 border-2 border-[var(--card-border)] p-3 font-black uppercase italic tracking-widest hover:-translate-y-0.5 hover:shadow-md transition-all" onClick={() => onReturn && onReturn({ type: 'follow_up', details: formData })}>STOP / FOLLOW UP</button>
                </div>
              )}
            </div>
          </div>,
      ...
      <div className="flex-1 h-full overflow-y-auto px-8 lg:px-12 pb-32 hide-scrollbar w-full pt-4">
        ...
        <div className="mb-6 flex">
          <button 
            ...
            className="flex items-center gap-2 px-6 py-2 bg-[var(--card-bg)]/80 border border-[var(--card-border)] text-[var(--text-base)] hover:text-[#FFE600] hover:bg-[var(--card-bg)]/60 transition-all font-semibold tracking-wide text-xl tracking-widest shadow-sm transition-all duration-300 - shrink-0"
          >
            &larr; Return to Dispatch
          </button>
        </div>
          {/* LEAD CONTEXT (Always Visible) */}
          <div className="mb-6 p-6 bg-[var(--card-bg)]/60 border border-[var(--card-border)] shadow-md relative rounded-none flex flex-col gap-4">
            <div className="absolute inset-0 opacity-10  [background-size:16px_16px]"></div>
            
            {/* NEW: Situational Awareness Header */}
            <div className="relative z-10 flex justify-between items-center bg-[var(--card-bg)]/80/10 p-2 border-2 border-[var(--card-border)]">
              <div className="flex gap-2 items-center flex-wrap">
                <span className="text-xs font-black tracking-widest text-gray-800 uppercase font-semibold tracking-wide mr-2">Distress / Motivation:</span>
                {[...(activeLead?.distressMarkers || []), ...(formData.distressMarkers || [])].length === 0 && !activeLead && (
                  <span className="bg-[var(--card-bg)]/60 text-[var(--text-base)] px-3 py-1 text-xs font-bold uppercase tracking-wider border-2 border-[var(--card-border)] shadow-md transition-all duration-300 -">
                    [PROBATE]
                  </span>
                )}
                {[...(activeLead?.distressMarkers || []), ...(formData.distressMarkers || [])].map((marker, idx) => (
                  <span key={idx} className="bg-[var(--card-bg)]/60 text-[var(--text-base)] px-3 py-1 text-xs font-bold uppercase tracking-wider border-2 border-[var(--card-border)] shadow-md transition-all duration-300 - flex items-center gap-1">
                    [{marker}]
                    {formData.distressMarkers?.includes(marker) && (
                      <button onClick={() => updateForm('distressMarkers', formData.distressMarkers.filter(m => m !== marker))} className="hover:text-gray-800 ml-1">&times;</button>
                    )}
                  </span>
                ))}
              ...
              <div className="flex gap-4 items-center bg-[var(--card-bg)] border-2 border-[var(--card-border)] px-4 py-1 shadow-md transition-all duration-300 ">
                <span className="text-xs font-bold text-[#D4AF37] uppercase tracking-widest">
                  <span className="text-gray-800">Last Contacted:</span> {activeLead?.lastContacted || '2023-10-24 (Hung Up)'}
                </span>
                <span className="text-xs font-bold text-gray-800 uppercase tracking-widest border-l-2 border-[var(--card-border)] pl-4">
                  Attempts: <span className="text-[#D4AF37]">{activeLead?.contactAttempts || '3'}</span>
                </span>
              </div>
            </div>

            <div className="grid grid-cols-4 gap-6 relative z-10 mt-2">
              <div>
                <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Lead Source</label>
                <select value={formData.activeSource} onChange={e => updateForm('activeSource', e.target.value)} className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md focus:translate-y-[-2px] transition-all cursor-pointer shadow-md">
              ...
              <div>
                <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">
                  ...
                  {formData.entityName && <span className="text-[#D4AF37] drop-shadow-md">✓ Saved</span>}
                </label>
                <select value={formData.manualEntityType} onChange={e => { updateForm('manualEntityType', e.target.value); }} className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md mb-2">
                ...
                {formData.manualEntityType !== 'INDIVIDUAL' && (
                  <input type="text" placeholder="Entity Name..." value={formData.entityName || ''} onChange={e => updateForm('entityName', e.target.value)} className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md" />
                )}
              </div>
              <div>
                ...
                <select value={formData.expectedPropertyType || 'standard single-family home'} onChange={e => { updateForm('expectedPropertyType', e.target.value); }} className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-2 text-gray-800 font-black uppercase text-xs text-ellipsis overflow-hidden whitespace-nowrap focus:outline-none focus:shadow-md">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input type="number" value={formData.arv || ''} onChange={e => { updateForm('arv', e.target.value); }} placeholder="0" className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 pl-7 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md" />
                  </div>
                  <div className="flex flex-col gap-1 w-16 shrink-0">
                    <button 
                      className={clsx("flex-1 text-[10px] font-black uppercase border-2 border-[var(--card-border)] tracking-tighter leading-none", formData.compConfidence === 'High' ? "bg-[#D4AF37] text-gray-800 shadow-inner" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")} 
                      onClick={() => updateForm('compConfidence', formData.compConfidence === 'High' ? null : 'High')}
                    >
                      HIGH
                    </button>
                    <button 
                      className={clsx("flex-1 text-[10px] font-black uppercase border-2 border-[var(--card-border)] tracking-tighter leading-none", formData.compConfidence === 'Low' ? "bg-[var(--card-bg)]/60 text-[var(--text-base)] shadow-inner" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")} 
          ...
          <div className="mb-6 p-6 bg-[var(--card-bg)]/60 border border-[var(--card-border)] shadow-md relative rounded-none">
            <div className="absolute inset-0 opacity-10  [background-size:16px_16px]"></div>
            <h4 className="text-gray-800 font-black text-2xl uppercase tracking-widest mb-4 flex items-center gap-3 relative z-10 font-semibold tracking-wide italic transition-all duration-300 -"><Database size={28} className="text-[#D4AF37]" /> PUBLIC RECORDS SYNC</h4>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
              <div>
                <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Property Sqft</label>
                <input type="number" value={formData.sqft || ''} onChange={e => updateForm('sqft', e.target.value)} placeholder="e.g. 1500" className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md focus:translate-y-[-2px] transition-all placeholder-gray-500 shadow-md" />
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  <input type="number" value={formData.lotSize || ''} onChange={e => updateForm('lotSize', e.target.value)} placeholder="0" className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md focus:translate-y-[-2px] transition-all placeholder-gray-500 shadow-md" />
                  <div className="flex flex-col gap-1 w-16">
                    <button 
                      className={clsx("flex-1 text-[10px] font-black uppercase border-2 border-[var(--card-border)]", formData.lotSizeMeasure === 'SQFT' ? "bg-[var(--card-bg)]/60 text-[var(--text-base)] shadow-inner" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")} 
                      onClick={() => {
                    ...
                    <button 
                      className={clsx("flex-1 text-[10px] font-black uppercase border-2 border-[var(--card-border)]", formData.lotSizeMeasure === 'ACRES' ? "bg-[var(--card-bg)]/60 text-[var(--text-base)] shadow-inner" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")} 
                      onClick={() => {
              ...
              <div>
                <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">
                  Year Built 
                  {formData.yearBuilt && (
                    <span className="ml-2 bg-[var(--card-bg)]/60 text-[var(--text-base)] px-2 py-0.5 text-xs font-medium rounded-sm shadow-md transition-all duration-300 - inline-block">
                      (Age: {new Date().getFullYear() - Number(formData.yearBuilt)} yrs)
                    </span>
                  )}
                </label>
                <input type="number" value={formData.yearBuilt || ''} onChange={e => updateForm('yearBuilt', e.target.value)} placeholder="e.g. 1990" className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md focus:translate-y-[-2px] transition-all placeholder-gray-500 shadow-md" />
              </div>
            </div>
          </div>
        {activeLead?.isManual && (
                ...
                <div className="flex gap-3">
                  {isLoaded ? (
                    <Autocomplete onLoad={(auto) => autocompleteRef.current = auto} onPlaceChanged={handlePlaceChanged} className="flex-1">
                      <input type="text" value={formData.manualAddress} onChange={e => updateForm('manualAddress', e.target.value)} className="w-full bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border-2 border-white/50 p-3 text-[var(--text-base)] font-medium focus:border-[#E5C158] focus:ring-1 focus:ring-[#E5C158]/30 outline-none placeholder-gray-600" placeholder="Search Google Maps..." />
                    </Autocomplete>
                  ) : (
                    <input type="text" value={formData.manualAddress} onChange={e => updateForm('manualAddress', e.target.value)} className="w-full bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border-2 border-white/50 p-3 text-[var(--text-base)] font-medium focus:border-[#E5C158] focus:ring-1 focus:ring-[#E5C158]/30 outline-none placeholder-gray-600" placeholder="123 Main St..." />
                  )}
          ...
          <div className="flex flex-col gap-6">
            ...
            {formData.isVoicemail && (
              <div className="flex flex-col mb-2 animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[var(--text-muted)]">Agent (Voicemail Script)</span>
                <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-6">
            ...
            {!formData.isVoicemail && !formData.isHostile && (
              <div className="flex flex-col mb-2 animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Direct Opener)</span>
                <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-6">
                  {formData.activeSource === 'Agent Outreach' 
            ...
            {!formData.isVoicemail && !formData.isHostile && formData.activeSource !== 'Agent Outreach' && (
              <div className="p-4 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl mt-4">
                <p className="text-xs font-medium mb-3 text-gray-800 uppercase">Seller Responses & Pushbacks (Click again to untoggle)</p>
                <div className="flex flex-wrap gap-3 mb-4">
                  <button className={clsx("px-5 py-2 border-2 border-[var(--card-border)] text-sm font-semibold tracking-wide tracking-widest uppercase transition-all hover:-translate-y-0.5 shadow-md transition-all duration-300 -", formData.introResponse === 'Normal' ? "bg-[#D4AF37] text-gray-800 border-[var(--card-border)] transition-all duration-300 -" : "text-gray-800 border-[var(--card-border)] hover:text-gray-800")} onClick={() => { updateForm('introResponse', formData.introResponse === 'Normal' ? null : 'Normal'); updateForm('activeObjection', null); }}>Normal Response</button>
                  ...
                  <button className={clsx("px-5 py-2 border-2 border-[var(--card-border)] text-sm font-semibold tracking-wide tracking-widest uppercase transition-all hover:-translate-y-0.5 shadow-md transition-all duration-300 -", formData.introResponse === 'Busy' ? "bg-[var(--card-bg)]/60 text-gray-800 border-[var(--card-border)] transition-all duration-300 -" : "text-gray-800 border-[var(--card-border)] hover:text-gray-800")} onClick={() => { updateForm('introResponse', formData.introResponse === 'Busy' ? null : 'Busy'); updateForm('activeObjection', null); }}>"I'm Busy / Call Later"</button>
                  <button className={clsx("px-5 py-2 border-2 border-[var(--card-border)] text-sm font-semibold tracking-wide tracking-widest uppercase transition-all hover:-translate-y-0.5 shadow-md transition-all duration-300 -", formData.introResponse === 'Robot' ? "bg-[var(--card-bg)]/60 text-gray-800 border-[var(--card-border)] neon-glow-cyan transition-all duration-300 -" : "text-gray-800 border-[var(--card-border)] hover:text-gray-800")} onClick={() => { updateForm('introResponse', formData.introResponse === 'Robot' ? null : 'Robot'); updateForm('activeObjection', null); }}>"Are you a robot / scam?"</button>
                  <button className={clsx("px-5 py-2 border-2 border-[var(--card-border)] text-sm font-semibold tracking-wide tracking-widest uppercase transition-all hover:-translate-y-0.5 shadow-md transition-all duration-300 -", formData.introResponse === 'PrematurePrice' ? "bg-accent-success text-gray-800 border-[var(--card-border)] transition-all duration-300 -" : "text-gray-800 border-[var(--card-border)] hover:text-gray-800")} onClick={() => { updateForm('introResponse', formData.introResponse === 'PrematurePrice' ? null : 'PrematurePrice'); updateForm('activeObjection', null); }}>"Just tell me your price"</button>
                ...
                {formData.introResponse === 'Normal' && (
                  <div className="flex flex-col mt-4 animate-slideIn">
                    <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Introduction)</span>
                    <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                ...
                {formData.introResponse === 'Who' && (
                  <div className="flex flex-col mt-4 animate-slideIn">
                    <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Rebuttal)</span>
                    <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                ...
                {['No', 'NotOwner'].includes(formData.introResponse) && (
                  <div className="flex flex-col mt-4 animate-slideIn">
                    <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Pivot)</span>
                    <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                ...
                {formData.introResponse === 'Busy' && (
                  <div className="flex flex-col mt-4 animate-slideIn">
                    <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Time Respect)</span>
                    <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                ...
                {formData.introResponse === 'Robot' && (
                  <div className="flex flex-col mt-4 animate-slideIn">
                    <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Identity)</span>
                    <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                ...
                {formData.introResponse === 'PrematurePrice' && (
                  <div className="flex flex-col mt-4 animate-slideIn">
                    <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Integrity)</span>
                    <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                    ...
                    <div className="mt-6 flex justify-end">
                      <button 
                        ...
                        className="px-6 py-3 bg-[var(--card-bg)]/60 hover:bg-[var(--card-bg)]/60/80 text-gray-800 font-semibold tracking-wide text-xl tracking-widest uppercase rounded-xl border-2 border-[var(--card-border)] shadow-md transition-all duration-300 transition-transition-all duration-300 hover:-translate-y-0.5"
                      >
            ...
            {!formData.isVoicemail && !formData.isHostile && formData.activeSource !== 'Agent Outreach' && (formData.introResponse || formData.activeObjection) && (
              <div className="flex flex-col mt-4 animate-slideIn">
                <div className="p-4 bg-[var(--card-bg)]/40 border border-[var(--card-border)] rounded-xl animate-slideIn">
                  <p className="text-xs font-medium mb-3 text-gray-800 uppercase">Would they consider an offer?</p>
                  ...
                  {formData.offerResponse === 'Yes' && (
                    <div className="flex flex-col mt-4 animate-slideIn">
                      <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Price Discovery)</span>
                      <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                    ...
                    <div className="flex flex-col mt-4 animate-slideIn">
                      ...
                      <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                        ...
                        <div className="mt-3 flex gap-2">
                          ...
                          <button className={clsx("px-3 text-xs rounded border transition-colors bg-[var(--card-bg)]", formData.refusedPrice ? "bg-red-600 border-red-600 text-gray-800" : "border-[var(--card-border)] text-gray-800")} onClick={() => updateForm('refusedPrice', !formData.refusedPrice)}>Refused Price</button>
                        </div>
                  ...
                  {formData.offerResponse === 'No' && (
                    <div className="flex flex-col mt-4 animate-slideIn">
                      <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Disqualify / Pivot)</span>
                      <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
        ...
        {renderPillar(2, "Property Details & Occupancy", <Home size={20} />, (
          <div className="flex flex-col gap-6">
            <div className="flex flex-col mb-2 animate-slideIn">
               <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Property Type)</span>
               <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-8">
            ...
            {formData.propertyType && formData.propertyType !== 'Multi-Family' && formData.propertyType !== 'Land' && (
              <>
                {/* 1. DECISION MAKERS */}
                <div className="flex flex-col mb-2 animate-slideIn">
                   <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Decision Makers)</span>
                       ...
                       <div className="flex flex-wrap gap-3 mb-4">
                         {['Sole Owner', 'Spouse/Partner', 'Trust/Probate/Multiple'].map(dm => (
                          ...
                          <div className="mt-4 p-4 border border-[var(--card-border)] bg-[var(--card-bg)] shadow-md relative">
                            <div className="mb-6">
                              <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Stewardship Alignment)</span>
                              <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                            ...
                            <div className="mb-6">
                              <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Total Decision Makers</label>
                              <input 
                                type="number" 
                                min="2" 
                                max="10" 
                                ...
                                className="w-24 bg-gray-100 border border-[var(--card-border)] rounded-none p-3 text-gray-800 font-black text-center text-lg focus:outline-none focus:shadow-md"
                                placeholder="#"
                              />
                            </div>
                            ...
                            {formData.dmCount >= 2 && (
                              <div className="animate-slideIn border-t-4 border-[var(--card-border)] pt-6">
                                <div className="mb-6">
                                  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Relationship Mapping)</span>
                                  <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none">
                                  ...
                                  <div className="flex flex-wrap gap-3 mb-4">
                                    {Array.from({ length: formData.dmCount }).map((_, i) => (
                                      <button 
                                        ...
                                        className={clsx("px-4 py-2 font-black border border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 uppercase text-xs tracking-wider", (formData.activeDm || 1) === i+1 ? "bg-[var(--card-bg)]/60 text-gray-800" : "bg-[var(--card-bg)] text-gray-800")}
                                      >
                                        DM {i+1}
                                      </button>
                                    ))}
                                ...
                                <div className="bg-gray-100 p-4 border border-[var(--card-border)] shadow-inner">
                                  ...
                                  <div className="flex flex-wrap gap-3 mb-4">
                                    {['Sibling', 'Ex-Spouse', 'Business Partner', 'Heir', 'Parent', 'Child', 'Attorney', 'Trustee', 'Executor'].map(rel => (
                                      <button 
                                        ...
                                        className={clsx("font-black text-xs uppercase px-4 py-2 border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5", formData[`dmRel_${formData.activeDm || 1}`] === rel ? "bg-[var(--card-bg)]/60 text-[var(--text-base)] transition-all duration-300 -" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                                      >
                                        {rel}
                                      </button>
                                    ))}
                              ...
                              <div className="animate-slideIn border-t-4 border-[var(--card-border)] pt-6 mt-6">
                                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Trust/Probate Follow-up)</span>
                                <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-6">
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-4">
                                  <div>
                                    <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Probate Started?</label>
                                    <div className="flex gap-2">
                                      {['Yes', 'No'].map(opt => (
                                        <button 
                                          key={opt} 
                                          onClick={() => updateForm('probateStarted', formData.probateStarted === (opt === 'Yes') ? null : (opt === 'Yes'))}
                                          className={clsx("flex-1 px-4 py-2 font-black border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 text-xs uppercase", formData.probateStarted === (opt === 'Yes') ? "bg-[var(--card-bg)]/60 text-gray-800" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                                        >
                                          {opt}
                                        </button>
                                      ))}
                                  ...
                                  <div>
                                    <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Executor / Admin Name</label>
                                    <input 
                                      type="text" 
                                      placeholder="Name..." 
                                      value={formData.executor || ''} 
                                      onChange={e => updateForm('executor', e.target.value)} 
                                      className="w-full bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all"
                                    />
                                  </div>
                                </div>
                              </div>
                ...
                {formData.decisionMakers && (
                  <>
                    <div className="flex flex-col mb-2 animate-slideIn">
                       <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Property Specs)</span>
                       <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-8">
                         <span className="font-bold">
                      ...
                      <div className="grid grid-cols-2 gap-6 mt-2">
                         <div>
                           ...
                           <input type="number" value={formData.sqft} onChange={e => updateForm('sqft', e.target.value)} className="w-full bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all" />
                         </div>
                         <div>
                           ...
                           <input type="number" value={formData.lotSize} onChange={e => updateForm('lotSize', e.target.value)} className="w-full bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all mb-1" />
                           {formData.lotSize && (
                ...
                {formData.decisionMakers && (
                  <>
                    <div className="flex flex-col mb-2 animate-slideIn">
                       <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Occupancy)</span>
                       <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-8">
                         <span className="font-bold">
                        ...
                        {['Owner Occupied', 'Tenant Occupied', 'Vacant'].map(occ => (
                          <button key={occ} onClick={() => updateForm('occupancy', formData.occupancy === occ ? '' : occ)} className={clsx("", formData.occupancy === occ ? "bg-[var(--card-bg)]/60 text-gray-800 border border-[var(--card-border)] font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform")}>{occ}</button>
                            ...
                            {formData.occupancy === 'Tenant Occupied' && (
                              <div className="mt-6 p-4 border border-[var(--card-border)] bg-gray-100 shadow-inner animate-slideIn">
                                <div className="mb-4">
                                  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Tenant Dignity)</span>
                                  <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-6">
                                    <span className="font-bold">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                  <div>
                                    <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Rent Amount</label>
                                    <div className="relative">
                                      ...
                                      <input 
                                        ...
                                        className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-none p-3 pl-7 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md"
                                      />
                                    </div>
                                  </div>
                                  <div>
                                    <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Lease Type</label>
                                    <div className="flex gap-2">
                                      {['Month-to-Month', 'Annual'].map(lt => (
                                        <button 
                                          key={lt} 
                                          onClick={() => updateForm('sfLeaseType', formData.sfLeaseType === lt ? '' : lt)} 
                                          className={clsx("flex-1 px-2 py-2 font-black border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 text-xs uppercase", formData.sfLeaseType === lt ? "bg-[var(--card-bg)]/60 text-[var(--text-base)]" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                                        >
                                          {lt}
                                        </button>
                                      ))}
                                ...
                                <div className="mt-4">
                                  <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Tenant Relationship</label>
                                  <div className="flex gap-2">
                                    {['Standard Renter', 'Family Member', 'Friend'].map(rel => (
                                      <button 
                                        key={rel} 
                                        onClick={() => updateForm('sfTenantRel', formData.sfTenantRel === rel ? '' : rel)} 
                                        className={clsx("flex-1 px-2 py-2 font-black border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 text-[10px] uppercase", formData.sfTenantRel === rel ? "bg-[var(--card-bg)]/60 text-gray-800" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                                      >
                                        {rel}
                                      </button>
                                    ))}
            ...
            {formData.propertyType === 'Land' && (
               <div className="flex flex-col mb-2 animate-slideIn">
                  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Land Specs)</span>
                  <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-8">
                    <span className="font-bold">
                    ...
                    <div className="grid grid-cols-2 gap-6 mb-6">
                      <div>
                        <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Lot Size / Acreage</label>
                        <div className="flex bg-gray-200 rounded p-0.5 mb-2 w-max">
                          <button className={clsx("px-3 py-1 text-xs font-bold uppercase rounded", formData.landSizeMeasure === 'SQFT' ? "bg-[var(--card-bg)]/80 text-[var(--text-base)] shadow-sm" : "text-[var(--text-muted)] hover:bg-gray-300")} onClick={() => updateForm('landSizeMeasure', 'SQFT')}>SQFT</button>
                          <button className={clsx("px-3 py-1 text-xs font-bold uppercase rounded", formData.landSizeMeasure === 'ACRES' ? "bg-[var(--card-bg)]/80 text-[var(--text-base)] shadow-sm" : "text-[var(--text-muted)] hover:bg-gray-300")} onClick={() => updateForm('landSizeMeasure', 'ACRES')}>ACRES</button>
                        </div>
                        <input 
                          type="number" 
                          value={formData.landSize || ''} 
                          onChange={e => updateForm('landSize', e.target.value)} 
                          className="w-full bg-gray-100 border border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-black placeholder-gray-500 focus:outline-none focus:border-[#E5C158] focus:ring-1 focus:ring-[#E5C158]/30 focus:shadow-md" 
                    ...
                    <div className="flex flex-col gap-4">
                      <div>
                        <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Utilities Access</label>
                        <div className="flex flex-wrap gap-3 mb-4">
                          {['City Water/Sewer', 'Well/Septic', 'No Utilities'].map(opt => (
                            <button 
                              key={opt} 
                              onClick={() => updateForm('landUtilities', formData.landUtilities === opt ? '' : opt)} 
                              className={clsx("px-4 py-2 font-black border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 text-xs uppercase", formData.landUtilities === opt ? "bg-[var(--card-bg)]/60 text-gray-800" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                            >
                              {opt}
                            </button>
                          ))}
                      ...
                      <div>
                        <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Road Access</label>
                        <div className="flex flex-wrap gap-3 mb-4">
                          {['Paved Road', 'Dirt Road', 'Landlocked / No Legal Access'].map(opt => (
                            <button 
                              key={opt} 
                              onClick={() => updateForm('landAccess', formData.landAccess === opt ? '' : opt)} 
                              className={clsx("px-4 py-2 font-black border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 text-xs uppercase", formData.landAccess === opt ? "bg-[var(--card-bg)]/60 text-gray-800" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                            >
                              {opt}
                            </button>
                          ))}
                      ...
                      <div>
                        <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Topography</label>
                        <div className="flex flex-wrap gap-3 mb-4">
                          {['Flat / Buildable', 'Sloped', 'Steep / Mountainous'].map(opt => (
                            <button 
                              key={opt} 
                              onClick={() => updateForm('landTopo', formData.landTopo === opt ? '' : opt)} 
                              className={clsx("px-4 py-2 font-black border-2 border-[var(--card-border)] shadow-md transition-transition-all duration-300 hover:-translate-y-0.5 text-xs uppercase", formData.landTopo === opt ? "bg-[var(--card-bg)]/60 text-[var(--text-base)]" : "bg-[var(--card-bg)] text-gray-800 hover:bg-gray-200")}
                            >
                              {opt}
                            </button>
                          ))}
            ...
            {formData.propertyType === 'Multi-Family' && (
              <div className="flex flex-col mb-2 animate-slideIn" style={{ marginTop: '1.5rem', animation: 'slideInUp 0.3s ease' }}>
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Multi-Family Phase 1 & 2)</span>
                <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-8">
                  <span className="font-bold">
                  ...
                  <div style={{ marginTop: '15px', background: 'transparent', padding: '15px', borderRadius: '12px', borderLeft: '4px solid #3b82f6' }}>
                    ...
                    <div className="mb-6">
                      <div className="text-sm text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Building Configuration</div>
                    ...
                    {formData.mfBuildingConfig === 'Multiple Detached Buildings' && (
                      <div className="mb-6 animate-slideIn">
                        <div className="text-sm text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Structure Consistency</div>
                        <div className="flex flex-wrap gap-3">
                    ...
                    {formData.mfBuildingConfig === 'Main House + ADU/Conversion' && (
                      <div className="mb-6 animate-slideIn flex flex-col gap-4">
                        <div>
                          <div className="text-sm text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">ADU Origin</div>
                          <div className="flex flex-wrap gap-3">
                        ...
                        {formData.mfAduOrigin !== '' && (
                          <div className="animate-slideIn">
                            <div className="text-sm text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">City Permits / Legality</div>
                            <div className="flex flex-wrap gap-3">
                        ...
                        {formData.mfAduLegality !== '' && (
                          <div className="animate-slideIn">
                            <div className="text-sm text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Utility Metering</div>
                            <div className="flex flex-wrap gap-3">
                    ...
                    {((formData.mfBuildingConfig === '1 Single Building') || (formData.mfBuildingConfig === 'Multiple Detached Buildings' && formData.mfStructureConsistency !== '')) && (
                      <div className="mb-6 animate-slideIn">
                        <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Total Units</label>
                        <input 
                          type="number" 
                          min="2" 
                    ...
                    {formData.mfUnitCount > 4 && (
                      <div className="mb-6 animate-slideIn border border-[var(--card-border)] bg-[var(--card-bg)] p-6 shadow-md">
                        <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Commercial Summary)</span>
                        <div className="relative bg-[var(--card-bg)]/50 backdrop-blur-md backdrop-blur-md border border-white/50 p-6 shadow-md text-xl font-medium text-[var(--text-base)] leading-relaxed rounded-2xl rounded-tl-none mb-6">
                          <span className="font-bold">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                          <div>
                            <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Total Monthly Gross Rent</label>
                            <div className="relative">
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] font-bold">$</span>
                              <input 
                                type="number" 
                                ...
                                className="w-full bg-gray-100 border border-[var(--card-border)] rounded-none p-3 pl-7 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md"
                              />
                            </div>
                          </div>
                          <div>
                            <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Current Vacancy Rate</label>
                            <div className="relative">
                              <input 
                                type="number" 
                                placeholder="e.g. 10" 
                                ...
                                className="w-full bg-gray-100 border border-[var(--card-border)] rounded-none p-3 pr-7 text-gray-800 font-black uppercase text-sm focus:outline-none focus:shadow-md"
                              />
                      ...
                      <div className="flex flex-col gap-4 mb-6">
                        {formData.mfUnitsData.map((unit, index) => (
                          <div key={index} className="bg-[#111] border-2 border-[#3b82f6] p-4 rounded-lg shadow-md">
                            <div className="text-[#00E5FF] font-medium uppercase tracking-widest mb-3 border-b border-gray-700 pb-2">
                              {formData.mfBuildingConfig === 'Main House + ADU/Conversion' 
                            ...
                            <div className="mb-4">
                              <div className="text-xs text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Layout</div>
                              <div className="flex flex-wrap gap-3 mb-4">
                            ...
                            <div className="mb-4">
                              <div className="text-xs text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Condition</div>
                              <div className="flex flex-wrap gap-3 mb-4">
                            ...
                            <div className="mb-2">
                              <div className="text-xs text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Occupancy Status</div>
                              <div className="flex flex-wrap gap-3 mb-4">
                                {['Owner Occupied', 'Tenant Occupied', 'Vacant'].map(occ => (
                            ...
                            {unit.occupancy === 'Tenant Occupied' && (
                              <div className="mt-4 p-3 bg-[var(--card-bg)]/80 border border-gray-800 rounded flex flex-col gap-3">
                                <div>
                                  <div className="text-[10px] text-[var(--text-muted)] mb-1 uppercase tracking-widest font-medium">Current Rent $</div>
                                ...
                                <div>
                                  <div className="text-[10px] text-[var(--text-muted)] mb-1 uppercase tracking-widest font-medium">Lease Type</div>
                                  <div className="flex gap-2">
                                    {['MTM', 'Annual'].map(lt => (
                                ...
                                <div>
                                  <div className="text-[10px] text-[var(--text-muted)] mb-1 uppercase tracking-widest font-medium">Payment Status</div>
                                  <div className="flex gap-2">
                      ...
                      <div className="mt-4 border-t border-gray-700 pt-4">
                        ...
                        {formData.mfUtilityMetering === 'Landlord Pays Some/All' && (
                          <div className="mt-4 p-4 bg-[var(--card-bg)]/80 border border-gray-800 rounded-lg">
                            <div className="text-xs text-[var(--text-muted)] mb-2 uppercase tracking-widest font-medium">Which Utilities?</div>
                            <div className="flex flex-wrap gap-3 mb-4">
                              {['Water', 'Sewer', 'Trash', 'Gas', 'Electric', 'Landscaping'].map(util => (
                                <button key={util} 
                                  className={`px-4 py-1.5 border font-medium text-xs shadow-[2px_2px_0px_#fff]  ${formData.mfOwnerUtilities.includes(util) ? 'bg-[var(--card-bg)]/60 text-gray-800 border-[#FFE600] shadow-[2px_2px_0px_#FFE600]' : 'bg-[var(--card-bg)]/80 border-gray-600 text-gray-300'} transition-all`} 
                                  onClick={() => {
                            ...
                            <div>
                              <div className="text-[10px] text-[var(--text-muted)] mb-1 uppercase tracking-widest font-medium">Est. Monthly Utility Cost $</div>
...
{formData.propertyType === 'Condo/Townhome' && (
            ...
            {isTenant && formData.propertyType !== 'Multi-Family' && (
              <div className="flex flex-col mb-2 animate-slideIn" style={{ marginTop: '1.5rem', animation: 'slideInUp 0.3s ease' }}>
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Tenant Follow-up)</span>
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--card-border)] box-border shadow-[8px_8px_0px_#FF00FF] p-6 mb-6 rounded-none mt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <input type="text" placeholder="RENT AMOUNT ($)..." value={formData.rentAmount || ''} onChange={(e) => updateForm('rentAmount', e.target.value)} className="bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all" />
                    <input type="text" placeholder="HOW DO THEY PAY? (ZELLE, CASH)..." value={formData.rentMethod || ''} onChange={(e) => updateForm('rentMethod', e.target.value)} className="bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all" />
                  </div>
                  ...
                  {(formData.tenantStatus?.includes('Someone is Behind') || formData.tenantStatus?.includes('Eviction Needed')) && (
                    <div className="border-t-4 border-[var(--card-border)] pt-4 mt-6">
                       ...
                       <div className="flex flex-wrap gap-4 items-center">
                         <input type="text" placeholder="AMOUNT BEHIND ($)..." value={formData.rentArrears || ''} onChange={(e) => updateForm('rentArrears', e.target.value)} className="bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all w-48" />
                  ...
                  {/* NEW OCCUPANCY DATA EXPANSION */}
                    <div className="border-t-4 border-[var(--card-border)] pt-4 mt-6">
                      <div className="text-gray-800 font-medium italic text-md mb-4">
                      ...
                      <div className="flex flex-col gap-4">
                        <div>
                          ...
                          <input type="text" placeholder="DEPOSIT HELD ($)..." value={formData.securityDeposit || ''} onChange={(e) => updateForm('securityDeposit', e.target.value)} className="bg-gray-100 border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-md transition-all w-full max-w-xs" />
                        </div>
                        ...
                        <div>
                          ...
                          {(formData.utilityStatus === 'LANDLORD PAYS W/S/T' || formData.utilityStatus === 'LANDLORD PAYS ALL') && (
                            <div className="mt-2 animate-slideIn">
                              <input type="text" placeholder="Landlord Cost / Mo ($)..." value={formData.landlordUtilityCost || ''} onChange={(e) => updateForm('landlordUtilityCost', e.target.value.replace(/[^0-9.]/g, ''))} className="bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-2 text-gray-800 uppercase font-medium placeholder-gray-500 focus:outline-none focus:border focus:shadow-md transition-all w-full max-w-xs" />
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          {(formData.section8Status === 'PARTIAL SECTION 8' || formData.section8Status === 'FULL SECTION 8') && (
                            <div className="mt-2 animate-slideIn">
                              <input type="text" placeholder="Section 8 Coverage / Mo ($)..." value={formData.section8Coverage || ''} onChange={(e) => updateForm('section8Coverage', e.target.value.replace(/[^0-9.]/g, ''))} className="bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-2 text-gray-800 uppercase font-medium placeholder-gray-500 focus:outline-none focus:border focus:shadow-md transition-all w-full max-w-xs" />
                            </div>
                          )}
                        </div>
                      </div>
            ...
            {isVacant && formData.propertyType !== 'Multi-Family' && (
              <div className="flex flex-col mb-2 animate-slideIn" style={{ marginTop: '1.5rem', animation: 'slideInUp 0.3s ease' }}>
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Vacant Follow-up)</span>
  ...
  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Transition)</span>
  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#D4AF37] text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-8">
  ...
  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Roof & AC)</span>
  ...
  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Reaction)</span>
  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#D4AF37] text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-8">
  ...
  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Plumbing & Electrical)</span>
  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#D4AF37] rounded-none mb-8">
    ...
    {formData.propertyType !== 'Multi-Family' && (
      <div className="mb-6">
        ...
        <div className="text-gray-800 font-medium italic text-md mt-6 mb-2">
  ...
  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-sm text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-8">
    "Okay, yeah, we usually end up having to repipe and rewire those older setups anyway, so that's not a deal breaker."
            ...
            <div className="flex flex-col mb-2 animate-slideIn" style={{ marginTop: '2rem' }}>
              <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Utilities & Exterior)</span>
                ...
                <div className="mb-6">
                  ...
                  {formData.septicSewer === 'Septic System' && (
                    <div className="mt-4 mb-4">
                      <div className="text-gray-800 font-medium italic text-md mt-4 mb-2">
                  ...
                  {(formData.solarSystem === 'Solar (Lease)' || formData.solarSystem === 'Solar (PPA)') && (
                    <div className="mt-4">
                      ...
                      <div className="grid grid-cols-2 gap-6 mt-4">
                        <input type="text" value={formData.solarCompany || ''} onChange={(e) => updateForm('solarCompany', e.target.value)} placeholder="SOLAR COMPANY NAME" className="bg-gray-100 border-2 border-[var(--card-border)] p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all col-span-2 sm:col-span-1" />
                        <input type="text" value={formData.solarMonthlyPayment || ''} onChange={(e) => updateForm('solarMonthlyPayment', e.target.value.replace(/[^0-9]/g, ''))} placeholder="MONTHLY PAYMENT $" className="bg-gray-100 border-2 border-[var(--card-border)] p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all col-span-2 sm:col-span-1" />
                        <input type="text" value={formData.solarPayoffAmount || ''} onChange={(e) => updateForm('solarPayoffAmount', e.target.value.replace(/[^0-9]/g, ''))} placeholder="TOTAL PAYOFF BALANCE $" className="bg-gray-100 border-2 border-[var(--card-border)] p-3 text-gray-800 uppercase font-medium placeholder-gray-600 focus:outline-none focus:border focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all col-span-2" />
                      </div>

                      <div className="text-gray-800 font-medium italic text-sm mt-4 mb-2 flex items-center gap-2">
            ...
            <div className="flex flex-col mb-2 animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Cosmetics & Inside)</span>
    <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#D4AF37] rounded-none mb-8">
      <div className="text-xl font-black italic text-gray-800 leading-relaxed mb-6">
      {formData.propertyType === 'Multi-Family' ? 
  ...
  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#FFE600] text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-8">
    "Gotcha. Sounds like it needs some pretty heavy cosmetic love. That's right up our alley."
        ...
        {formData.propertyType !== 'Multi-Family' && (
            ...
            <div className="flex flex-col mb-2 animate-slideIn" style={{ marginTop: '2rem' }}>
  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Structure & Risk)</span>
  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-sm rounded-none mb-8">
    <div className="text-xl font-black italic text-gray-800 leading-relaxed mb-6">
                    ...
                    <div className="bg-red-50 p-4 border border-[#E74C3C] animate-slideIn">
                      ...
                      {formData.redFlagFoundation === 'Minor Settling / Cracks' && (
                        <div className="mt-4 animate-slideIn">
                          <div className="text-gray-800 font-black italic mb-2 text-sm">"Got it, so just typical hairline cracks, nothing the city has ever been involved with or required underpinning for?"</div>
                          <div className="flex flex-wrap gap-3 mb-4">
                            {['Typical Settling (No Action)', 'Previously Repaired/Underpinned'].map(opt => (
                              <button key={opt} onClick={() => updateForm('redFlagFoundationDetails', opt)} className={clsx("", formData.redFlagFoundationDetails === opt ? "bg-[#FFFF00] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-3 py-1 cursor-pointer" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-600 font-bold text-xs uppercase px-3 py-1 cursor-pointer")}>{opt}</button>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {formData.redFlagFoundation === 'Major Sinking / Shifting' && (
                        <div className="mt-4 animate-slideIn">
                          <div className="text-gray-800 font-black italic mb-2 text-sm">"Since it's major shifting, has a structural engineer looked at it, or is the floor visibly slanting?"</div>
                          <div className="flex flex-wrap gap-3 mb-4">
                      ...
                      {formData.redFlagFire === 'Cosmetic / Smoke Only' && (
                        <div className="flex flex-wrap gap-2 mt-4 animate-slideIn">
                          {['Professionally Mitigated', 'Needs Smoke Remediation / Paint', 'Active Insurance Claim'].map(opt => (
                            <button key={opt} onClick={() => updateForm('redFlagFireDetails', opt)} className={clsx("", formData.redFlagFireDetails === opt ? "bg-[#FFFF00] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-3 py-1 cursor-pointer" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-600 font-bold text-xs uppercase px-3 py-1 cursor-pointer")}>{opt}</button>
                          ))}
                        </div>
                      )}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-[#fffdf0] border border-[var(--card-border)] p-6 shadow-md relative mt-2">
              ...
              <div className="flex flex-col mb-8 animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Timeline)</span>
                ...
                <div className="mb-2">
                  ...
                  {formData.timelineType && !formData.tenantStatus?.includes('Eviction Needed') && (
                    <div className="mt-6 animate-slideIn">
                      <div className="text-xl font-medium italic text-gray-800 leading-relaxed  mb-4">
                      ...
                      <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Motivation / Pain Points</label>
                      <div className="flex flex-wrap gap-3 mb-4">
                        {['Tired Landlord', 'Pre-Foreclosure', 'Divorce', 'Inherited / Probate', 'Job Relocation', 'Financial Distress', 'Downsizing', 'Just Want Cash'].map(opt => {
                          ...
                          return (
                            <button 
                              ...
                              className={clsx(
                                "px-4 py-2 text-xs font-bold uppercase transition-all", 
                                isSelected 
                                  ? "bg-[#E74C3C] text-[var(--text-base)] border border-[var(--card-border)] shadow-md" 
                                  : "bg-[var(--card-bg)] text-gray-800 border-2 border-[var(--card-border)] shadow-md hover:-translate-y-0.5"
                              )}
                            >
                              {opt}
                            </button>
              ...
              {/* RELOCATION */}
              <div className="flex flex-col mb-8 animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Relocation)</span>
                <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#FFFF00] text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-6">
                ...
                <div className="mb-2">
                  ...
                  {formData.relocationType && (
                    <input 
                      ...
                      className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--card-border)] p-3 text-gray-800 font-medium uppercase outline-none focus:border-white/50 shadow-md" 
                    />
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Cash to Move / Post-Possession)</span>
                <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#FFFF00] text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-6">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-[#fffdf0] border border-[var(--card-border)] p-6 shadow-md relative mt-2">
              ...
              <div className="flex flex-col mb-6 animate-slideIn">
                <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Transition to Financials)</span>
                ...
                {formData.freeAndClear === false && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-slideIn mb-6">
                    <div className="flex flex-col">
                      ...
                      <input type="text" value={formData.mortgageBalance || ''} onChange={(e) => updateForm('mortgageBalance', e.target.value.replace(/[^0-9]/g, ''))} placeholder="0" className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:border-white/50 focus:shadow-md transition-all" />
                    </div>
                    <div className="flex flex-col">
                      <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">2nd Position / HELOC $</label>
                      <input type="text" value={formData.secondPosition || ''} onChange={(e) => updateForm('secondPosition', e.target.value.replace(/[^0-9]/g, ''))} placeholder="0" className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:border-white/50 focus:shadow-md transition-all" />
                    </div>
                    <div className="flex flex-col">
                      <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Additional Liens $</label>
                      <input type="text" value={formData.thirdPosition || ''} onChange={(e) => updateForm('thirdPosition', e.target.value.replace(/[^0-9]/g, ''))} placeholder="0" className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-3 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:border-white/50 focus:shadow-md transition-all" />
                    </div>
                    <div className="flex flex-col">
            ...
            <div className="mt-6 pt-4 flex justify-center w-full">
              <button 
                ...
                className={clsx(
                  "w-full max-w-lg py-4 uppercase font-black italic text-2xl tracking-widest border border-[#000000] rounded-none shadow-md transition-all flex items-center justify-center gap-3",
        ...
        {activeSource !== 'Agent Outreach' && renderPillar(5, "The Offer (Pivot)", <HeartHandshake size={20} />, (
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-[#fffdf0] border border-[var(--card-border)] p-6 shadow-md relative mt-2">
              ...
              {(computedTotalDebt > 0 && formData.askingPrice && Number(formData.askingPrice.toString().replace(/[^0-9.-]+/g,"")) > displayMAO) ? (
                <div className="flex flex-col mb-2 animate-slideIn">
                  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (Creative Pivot)</span>
                ...
                <div className="flex flex-col mb-2 animate-slideIn">
                  <span className="text-xs font-medium mb-1 tracking-wider uppercase text-[#00E5FF]">Agent (The Pitch)</span>
                  <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-[8px_8px_0px_#D4AF37] text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-6">
              ...
              <div className="mt-8 border-t-4 border-[var(--card-border)] pt-6">
                 ...
                 <div className="flex flex-wrap gap-3 mb-4">
                   <button className={clsx("", formData.activeObjection === 'too_low' ? "bg-gradient-to-r from-[#CD7F32] via-[#E5C158] to-[#B8860B] text-gray-800 border border-[var(--card-border)] font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform")} onClick={() => handleRebuttalClick('too_low')}>
                     "That's too low"
                   </button>
                   <button className={clsx("", formData.activeObjection === 'no_terms' ? "bg-gradient-to-r from-[#CD7F32] via-[#E5C158] to-[#B8860B] text-gray-800 border border-[var(--card-border)] font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform")} onClick={() => handleRebuttalClick('no_terms')}>
                     "Not Open to Terms"
                   </button>
                   <button className={clsx("", formData.activeObjection === 'think_about_it' ? "bg-gradient-to-r from-[#CD7F32] via-[#E5C158] to-[#B8860B] text-gray-800 border border-[var(--card-border)] font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform")} onClick={() => handleRebuttalClick('think_about_it')}>
                     "I need to think about it"
                   </button>
                   <button className={clsx("", formData.activeObjection === 'other_offers' ? "bg-gradient-to-r from-[#CD7F32] via-[#E5C158] to-[#B8860B] text-gray-800 border border-[var(--card-border)] font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform")} onClick={() => handleRebuttalClick('other_offers')}>
                     "I have higher offers"
                   </button>
                   <button className={clsx("", formData.activeObjection === 'spouse' ? "bg-gradient-to-r from-[#CD7F32] via-[#E5C158] to-[#B8860B] text-gray-800 border border-[var(--card-border)] font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform" : "bg-[var(--card-bg)] border-2 border-[var(--card-border)] text-gray-800 font-black text-xs uppercase px-4 py-2 cursor-pointer shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-transform")} onClick={() => handleRebuttalClick('spouse')}>
                     "Need to talk to spouse"
                   </button>
                 </div>

                 {formData.activeObjection === 'no_terms' && (
                   <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] box-border max-w-full mr-4 p-6 shadow-sm text-xl font-black italic text-gray-800 leading-relaxed rounded-none mb-6">
                     ...
                     <div className="flex flex-col bg-gray-100 border-2 border-[var(--card-border)] p-4">
                       ...
                       {formData.thinkAboutItConcern === 'Something Else' && (
                         <input 
                           ...
                           className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] p-2 text-gray-800 uppercase font-medium outline-none focus:border-white/50"
                         />
                       )}
                     </div>
                   </div>
            ...
            <div className="flex flex-col mb-2 animate-slideIn bg-[var(--card-bg)] border border-[var(--card-border)] p-6 shadow-md mt-2">
              ...
              <div className="flex flex-col gap-4 mt-2">
                <div className="flex flex-col">
                  ...
                  <input type="text" placeholder="Legal Name(s) for Contract..." value={formData.legalName || ''} onChange={(e) => updateForm('legalName', e.target.value)} className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-4 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:bg-yellow-50 focus:shadow-md transition-all" />
                </div>
                
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Contact Email</label>
                  <input type="email" placeholder="Best Email Address..." value={formData.contactEmail || ''} onChange={(e) => updateForm('contactEmail', e.target.value)} className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-4 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:bg-yellow-50 focus:shadow-md transition-all" />
                </div>
                
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold text-xs uppercase tracking-wider block mb-1">Mailing Address</label>
                  <input type="text" placeholder="Current Mailing Address (if different)..." value={formData.mailingAddress || ''} onChange={(e) => updateForm('mailingAddress', e.target.value)} className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-4 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:bg-yellow-50 focus:shadow-md transition-all" />
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <input type="text" placeholder="Routing or Disbursement Instructions..." value={formData.routingInfo || ''} onChange={(e) => updateForm('routingInfo', e.target.value)} className="w-full bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-none p-4 text-gray-800 uppercase font-medium placeholder-gray-400 focus:outline-none focus:bg-yellow-50 focus:shadow-md transition-all" />
                </div>
              </div>
            </div>
