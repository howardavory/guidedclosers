...
const stateRegex = /mfUnitsData: \[\](?:,[\s\S]*?)mfUtilityCost: ''/g;
code = code.replace(stateRegex, "mfUnitsData: [], mfUtilityMetering: '', mfOwnerUtilities: [], mfUtilityCost: '', mfBuildingConfig: '', mfInteriorCondition: ''");

// 2. Update Pillar 1 transition text
const pillar1TransitionRegex1 = /"Got it, \$?\$\{Number\(\(formData\.askingPrice\|\|''\)\.toString\(\)\.replace\(\/\[\^0-9\.\-\]\+\/g,""\)\)\.toLocaleString\(\) \|\| formData\.askingPrice\}\. For us to see if we can make that number work, it's going to depend heavily on the condition and layout\."/g;
const pillar1TransitionRegex2 = /"No worries at all, I completely understand\. Usually when we buy properties, the exact number we can offer is going to depend heavily on the condition and layout\."/g;

code = code.replace(pillar1TransitionRegex1, `"Got it, $$\{Number((formData.askingPrice||'').toString().replace(/[^0-9.-]+/g,"")).toLocaleString() || formData.askingPrice}. For me to see if I can make that number work, it's going to depend heavily on the condition and layout."`);
code = code.replace(pillar1TransitionRegex2, `"Got it, no problem. For me to see if I can make the numbers work, it's going to depend heavily on the condition and layout."`);

// 3. Update Property Type Prompt
const propTypePromptRegex = /<span>"Now, just to make sure my file is completely accurate before we get into the weeds—public records are showing this as a standard single-family home\. Is that right, or are we looking at a multi-unit or mobile home\?"<\/span>/g;
code = code.replace(propTypePromptRegex, `<span>"Now, just to make sure my file is completely accurate before we dive into the specs—public records are showing this as a standard single-family home. Is that right, or are we looking at a multi-unit or mobile home?"</span>`);

// 4. Update Multi-Family Render Block (Phases 1-3)
                      ...
                      <div className="flex flex-col gap-4 mb-6">
                        {formData.mfUnitsData.map((unit, index) => (
                          <div key={index} className="bg-[#111] border-2 border-[#3b82f6] p-4 rounded-lg shadow-[4px_4px_0px_#000]">
                            ...
                            <div className="mb-2">
                              <div className="text-xs text-gray-400 mb-2 uppercase tracking-widest font-bold">Occupancy</div>
                              <div className="flex flex-wrap gap-2">
                            ...
                            {unit.occupancy === 'Tenant Occupied' && (
                              <div className="mt-4 p-3 bg-black border border-gray-800 rounded flex flex-col gap-3">
                                <div>
                                  <div className="text-[10px] text-gray-500 mb-1 uppercase tracking-widest font-bold">Rent Amount $</div>
...
console.log('Phases 1-3 Injected!');
